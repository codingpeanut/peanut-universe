...
void *sort_and_sum(void *arg) {
    ...
    printf("Thread %d sorted and summed column %d: sum = %ld\n", column + 1, column + 1, td->sum);
    pthread_exit(NULL);
}
...
void *merge_columns(void *arg) {
    ...
    fprintf(output_file, "\n"); // Newline after the merged data
    fclose(output_file);
    
    printf("Merged columns and wrote to output file.\n");
    free(merged_data);
    pthread_exit(NULL);
}
...
int main() {
    ...
    fclose(output_file);  // Clear previous content of output file
    ...
    // Create an array of pointers to pass to merge_columns
    ThreadData *thread_data_ptrs[NUM_COLUMNS];
    for (int i = 0; i < NUM_COLUMNS; i++) {
        thread_data_ptrs[i] = &thread_data[i];
    }

    pthread_create(&merge_thread, NULL, merge_columns, thread_data_ptrs);
