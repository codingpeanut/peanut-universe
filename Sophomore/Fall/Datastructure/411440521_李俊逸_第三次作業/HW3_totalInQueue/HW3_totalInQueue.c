...
int main() {
    ...
    // p1: assign k the last element
    random_number_generator(&q, 10, 0, 30);         // Use rand() % 10 + 0 to get 30 random numbers
    printq(&q, 10);                                 // 10 numbers in one line
    printf("p1: assign k the last element\n");
    int k = getElement(&q, FROM_BACK, 1, NO_RECOVER);
    printf("k = %d \n", k);
    printq(&q, 10);
    printf("\n");
    
    // p2: assign k the last element without changing the queue
    random_number_generator(&q, 10, 0, 30);
    printq(&q, 10);
    printf("p2: assign k the last element without changing the queue\n");
    k = getElement(&q, FROM_BACK, 1, RECOVER);
    printf("k = %d \n", k);
    printq(&q, 10);
    printf("\n");
    
    // p3: assign k the 2nd element from back
    random_number_generator(&q, 10, 0, 30);
    printq(&q, 10);
    printf("p3: assign k the 2nd element from back\n");
    k = getElement(&q, FROM_BACK, 2, NO_RECOVER);
    printf("k = %d \n", k);
    printq(&q, 10);
    printf("\n");
    
    // p4: assign k the 2nd element from back without changing the queue
    random_number_generator(&q, 10, 0, 30);
    printq(&q, 10);
    printf("p4: assign k the 2nd element from back without changing the queue\n");
    k = getElement(&q, FROM_BACK, 2, RECOVER);
    printf("k = %d \n", k);
    printq(&q, 10);
    printf("\n");
	// 411440521 JoshLee
