...
; 2(b)		display reversely the string characters in multiple-of-3 positions (assuming the last character starts from position 0)
; 0                    21   26 29     36   41    string_length = 42 (index: 0~41)
; 411440521-Lee-Midterm Exam in Spring 2024!
; ------------------------ print multiple-of-3 positions -------------------------
			LDR     r1, =StudentData    ; starting address of StudentData
			ADD		r1, #42				; rev_head_address(r1) = StudentData(r1) + string_length(#42)
			MOV		r2, #14				; print_counter = 43/3 = 14

			SUB		r1, #3				; first character
Loop2_Tx
            LDRB    r0, [r1], #-3       ; load character, decrement address
            CMP     r2, #0              ; print_counter greater than 0?
            BLNE    Transmit            ; send character to UAER0
			SUB		r2, #1				; print_counter--
            BNE     Loop2_Tx            ; continue if print_counter not equals to 0
; ------------------------ print multiple-of-3 positions -------------------------
