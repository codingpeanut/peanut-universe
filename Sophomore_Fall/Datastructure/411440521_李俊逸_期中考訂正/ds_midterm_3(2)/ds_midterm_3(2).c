 ...
 ********************************/
/** 3.1
Assume a queue is implemented by a circular array. 
Comment in front of the program in (2) the conditions respectively for an empty queue and a full queue if

(a) front points to two position before the queue head and rear points to two position after the queue tail.
Ans:
    int empty() { return rear == (front + 3) % SIZE; }
    int full() { return rear == (front + 3) % SIZE; }

(b) front points to two position after the queue head and rear points to two position before the queue tail.
Ans:
    int empty() { return rear == (front + 5) % SIZE; }
    int full() { return rear == (front + 5) % SIZE; }
 **/
...
// 411440521 JoshLee
void reverse_queue(CircularQueue *q) {        // reverse with recursion
    // base case
    if (emptyq(q))
        return;
...
int main() {
    ...
    // p1 Use rand()%100+1 to get 18 random numbers, output thenumbers (one by one, one space in between, and 9 numbersin one line) 
    ...
    // p2 Assign and output integer n the 10th element from the head of Q, leaving Q unchanged
    int n = getElement(&q, 1, 10, 1);
    printf("n = %d \n\n", n);
